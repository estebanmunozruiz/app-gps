...
const ui = {
  ...
  manualLocationBtn: document.querySelector("#manualLocationBtn"),
...
function setPosition(position) {
  ...
  if (!state.track.length || distanceBetween(state.track[state.track.length - 1], next) > 0.4) {
...
function updateMetrics() {
  ...
  if (!guidance) {
    ...
    if (!state.current) {
      ui.steerHint.textContent = "Inicia GPS o demo";
...
function getBounds() {
  ...
  const centerX = state.current ? state.current.x : (Math.min(...xs) + Math.max(...xs)) / 2;
  const centerY = state.current ? state.current.y : (Math.min(...ys) + Math.max(...ys)) / 2;
...
function startGps() {
  ...
  state.watchId = navigator.geolocation.watchPosition(
    (position) => {
      ...
      ui.manualLat.value = position.coords.latitude.toFixed(6);
      ui.manualLon.value = position.coords.longitude.toFixed(6);
    },
    (error) => {
      const messages = {
...
function useManualLocation() {
  const lat = Number(ui.manualLat.value);
  const lon = Number(ui.manualLon.value);

  if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
    ui.gpsStatus.textContent = "Coordenadas invalidas";
    ui.steerHint.textContent = "Ingresa latitud y longitud";
    return;
  }
  ...
  resetRun(false);
  ui.gpsStatus.textContent = "Ubicacion manual";
  ui.gpsStatus.className = "status-pill live";
  setPosition({
    lat,
    lon,
...
function startDemo() {
  ...
  resetRun(false);
  state.demoT = 0;
  ui.gpsStatus.textContent = "Demo activo";
  ui.gpsStatus.className = "status-pill demo";

  const base = { lat: -33.45, lon: -70.66 };
  state.demoTimer = setInterval(() => {
    state.demoT += 1;
    const metersX = Math.sin(state.demoT / 55) * 10 + state.demoT * 1.7;
    const metersY = Math.floor(state.demoT / 85) * 18 + Math.sin(state.demoT / 15) * 1.5;
    const lonMeters = 111320 * Math.cos((base.lat * Math.PI) / 180);
    setPosition({
      lat: base.lat + metersY / 111320,
      lon: base.lon + metersX / lonMeters,
      accuracy: 1.2,
      speed: 2.8,
      time: Date.now(),
    });

    if (state.demoT === 8) state.pointA = state.current;
    if (state.demoT === 45) state.pointB = state.current;
  }, 350);
}
...
ui.manualLocationBtn.addEventListener("click", useManualLocation);
