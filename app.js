...
const DEFAULT_MAP_CENTER = { lat: -33.45, lon: -70.66 };

function getSavedMapCenter() {
  try {
    const saved = JSON.parse(localStorage.getItem("gps-agricola-last-center") || "null");
...
function getMapOrigin() {
  return state.origin || getSavedMapCenter();
}
...
function metersToLatLon(point) {
  ...
  return {
    lat: origin.lat + point.y / latMeters,
    lon: origin.lon + point.x / lonMeters,
  };
}
...
function setPosition(position) {
  ...
  state.current = next;
  localStorage.setItem("gps-agricola-last-center", JSON.stringify({ lat: next.lat, lon: next.lon }));
  if (!state.userPanned) {
...
function latLonToMeters(position) {
  ...
  return {
    x: (position.lon - origin.lon) * lonMeters,
    y: (position.lat - origin.lat) * latMeters,
  };
}
